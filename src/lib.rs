...
fn parse_machine(input: &str) -> IResult<&str, NetrcMachine> {
    ...
    Ok((rest, NetrcMachine { machine: machine_name.to_string(), login, password, account, macdef }))
}
...
mod tests {
    ...
    fn init_logger() {
        ...
        let _ = log::set_logger(&TestLogger).map(|()| log::set_max_level(log::LevelFilter::Debug));
